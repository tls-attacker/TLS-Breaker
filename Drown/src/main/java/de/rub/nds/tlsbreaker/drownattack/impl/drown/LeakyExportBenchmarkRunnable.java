...
/**
 * Runnable for benchmarking the brute-force performance of ServerVerifyChecker in a multi-threaded
 * setup.
 */
class LeakyExportBenchmarkRunnable implements Runnable {
    ...
    public LeakyExportBenchmarkRunnable(
            SSL2CipherSuite cipherSuite, int firstByteFrom, int firstByteTo) {
    ...
    public void init(
            byte[] encrypted, byte[] baseMasterKey, byte[] challenge, byte[] sessionId, byte[] iv) {
